...
| `rerun.sbatch` | One array task: runs its share of the manifest, appending each result to `results/task_<id>.jsonl` |
...
```bash
python .../rerun.py manifest --design decision      # 1,190 runs in 20 tasks
sbatch --array=0-19 --export=ALL,DESIGN=decision .../rerun.sbatch
python .../rerun.py collect --design decision       # any time
```
...
- `collect` ends with the tasks that still have runs to do. Resubmit only those with the printed `--array=...` list. Failed runs are recorded with their error and are not retried; delete their lines from the results file to retry them.
