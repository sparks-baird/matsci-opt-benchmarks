...
# and `pip install matsci-opt-benchmarks` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "matsci-opt-benchmarks", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n matsci-opt-benchmarks -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "matsci-opt-benchmarks", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "matsci-opt-benchmarks", "YOUR_CONSOLE_SCRIPT"]
